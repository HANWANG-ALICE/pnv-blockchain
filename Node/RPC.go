...
func(p *PPoV) PostRecord(request *PostRecordRequest, basic *BasicMessage) error{
	...
	p.Node.txPool.PushbackTransaction(transactionHeader, &transaction)
	...
	basic.Message = "SUCCESS"
	result := map[string]string{
		"hash":string(base64.StdEncoding.EncodeToString(KeyManager.GetHash(item))),
	}
...
func(p *PPoV) GetBalance(request *GetBalanceMsg, basic *BasicMessage) error{
	...
	basic.Message = "SUCCESS"
	result := map[string]string{
		request.Address:string(value.(int)),
	}
